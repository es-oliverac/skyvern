...
class CaptchaSolutionNotFound(SkyvernException):
    def __init__(
        self,
        task_id: str | None = None,
        workflow_run_id: str | None = None,
        ...
        msg = "CAPTCHA solution not found."
        if task_id:
            msg += f" task_id={task_id}"
        if workflow_run_id:
            msg += f" workflow_run_id={workflow_run_id}"
        if captcha_type:
            msg += f" captcha_type={captcha_type}"
        if reason:
            msg += f" reason: {reason}"
        super().__init__(msg)


class CaptchaSolvingFailed(SkyvernException):
    def __init__(
        self,
        captcha_type: str,
        reason: str | None = None,
        ...
        msg = f"CAPTCHA solving failed for type {captcha_type}"
        if reason:
            msg += f". reason: {reason}"
        if error_code:
...
class CaptchaTimeout(SkyvernException):
    def __init__(
        self,
        captcha_type: str,
        timeout_seconds: int,
    ) -> None:
        msg = f"CAPTCHA solving timed out after {timeout_seconds}s for type {captcha_type}"
        super().__init__(msg)


class CaptchaDetectionFailed(SkyvernException):
    def __init__(
        self,
        reason: str | None = None,
    ) -> None:
        msg = "CAPTCHA detection failed"
        if reason:
            msg += f". reason: {reason}"
        super().__init__(msg)


class CaptchaInjectionFailed(SkyvernException):
    def __init__(
        self,
        captcha_type: str,
        ...
        msg = f"CAPTCHA solution injection failed for type {captcha_type}"
        if element_id:
