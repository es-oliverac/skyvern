...
async def handle_solve_captcha_action(
    ...
    """
    Handle CAPTCHA solving by integrating with 2Captcha service.

    Process:
    1. Check if CAPTCHA solving is enabled
    2. Detect CAPTCHA type and parameters from page
    3. Submit CAPTCHA to 2Captcha API
    4. Poll for solution with timeout
    5. Inject solution into page
    6. Verify solution was accepted
    """
    from skyvern.services.captcha_service import solve_captcha

    # If CAPTCHA solving is disabled, wait for manual resolution (current behavior)
    if not settings.CAPTCHA_SOLVING_ENABLED:
        LOG.warning(
            "CAPTCHA solving is disabled, waiting for manual resolution",
    ...
    # Attempt automatic CAPTCHA solving with 2Captcha
    try:
        LOG.info(
        ...
        solution = await solve_captcha(
            ...
            task_id=task.task_id,
            step_id=step.step_id,
            timeout=settings.CAPTCHA_SOLVING_TIMEOUT_SECONDS,
        )
        ...
        LOG.info(
            ...
            solving_time_seconds=solution.solving_time_seconds,
        )
        ...
        LOG.warning(
            "CAPTCHA solving timed out, falling back to manual resolution",
            task_id=task.task_id,
            step_id=step.step_id,
            error=str(e),
        )
        # Fall back to manual resolution on timeout
        await asyncio.sleep(30)
        return [ActionSuccess()]

    except CaptchaSolvingFailed as e:
        LOG.error(
        ...
        # Return failure but don't stop execution
        return [ActionFailure(exception=e, stop_execution_on_failure=False)]

    except CaptchaInjectionFailed as e:
        LOG.error(
            "CAPTCHA solution injection failed",
            task_id=task.task_id,
            step_id=step.step_id,
            error=str(e),
        )
        ...
        LOG.exception(
            "Unexpected error solving CAPTCHA",
            task_id=task.task_id,
            step_id=step.step_id,
        )
        return [ActionFailure(exception=e, stop_execution_on_failure=False)]
