...
Skyvern supports automatic CAPTCHA solving using the 2Captcha service to handle CAPTCHAs encountered during browser automation. This feature allows your workflows to continue seamlessly even when websites present CAPTCHA challenges.

### Supported CAPTCHA Types

- **reCAPTCHA v2**: The classic "I'm not a robot" checkbox and image challenges
- **reCAPTCHA v3**: Invisible CAPTCHA that runs in the background
- **hCaptcha**: Popular alternative to Google's reCAPTCHA
- **Text/Image CAPTCHAs**: Traditional text-based CAPTCHAs (coming soon)

### How It Works

When Skyvern encounters a CAPTCHA during task execution:

1. **Automatic Detection**: Skyvern detects the CAPTCHA type and extracts necessary parameters (site key, page URL)
2. **API Submission**: The CAPTCHA is submitted to 2Captcha's solving service
3. **Solution Polling**: Skyvern polls for the solution (typically 15-30 seconds)
4. **Token Injection**: The solution token is injected into the page
5. **Verification**: Skyvern verifies the solution was accepted
6. **Continuation**: Your workflow continues automatically

### Configuration

To enable automatic CAPTCHA solving, configure the following environment variables in your `.env` file:

```bash
# Enable/disable automatic CAPTCHA solving
CAPTCHA_SOLVING_ENABLED=true

# Your 2Captcha API key (get it from https://2captcha.com/settings)
TWOCAPTCHA_API_KEY=your_api_key_here

# Optional: Software key for faster processing
TWOCAPTCHA_SOFTWARE_KEY=

# Polling interval in seconds (default: 5)
CAPTCHA_POLLING_INTERVAL_SECONDS=5

# Maximum time to wait for solution in seconds (default: 120)
CAPTCHA_SOLVING_TIMEOUT_SECONDS=120
...
1. Visit [2Captcha.com](https://2captcha.com/) and create an account
2. Navigate to [Settings](https://2captcha.com/settings)
3. Copy your API key
4. Add it to your `.env` file: `TWOCAPTCHA_API_KEY=your_actual_api_key`

### Cost Considerations

2Captcha charges approximately **$2.99 per 1,000 reCAPTCHA v2 solves** as of 2025. Pricing varies by CAPTCHA type:

- reCAPTCHA v2: ~$2.99/1000 solves
- reCAPTCHA v3: ~$1.99/1000 solves
- hCaptcha: ~$2.99/1000 solves
- Text CAPTCHAs: ~$0.50/1000 solves

### Fallback Behavior

If CAPTCHA solving is disabled or fails, Skyvern will:
1. Log the failure
2. Wait 30 seconds for manual resolution
3. Continue the workflow

This ensures your workflows don't break completely if automatic solving fails.

### Example Usage

Once configured, CAPTCHA solving works automatically without any code changes:
...
skyvern = Skyvern()
task = await skyvern.run_task(
    prompt="Log in to the website and download the report",
    url="https://example.com/login"
)

# If a CAPTCHA appears during login, it will be solved automatically
```

### Troubleshooting

**CAPTCHA solving is not working:**
- Verify `CAPTCHA_SOLVING_ENABLED=true` in your `.env` file
- Check that your 2Captcha API key is valid
- Ensure you have sufficient balance in your 2Captcha account
- Check logs for specific error messages

**Solving is too slow:**
- Reduce `CAPTCHA_SOLVING_TIMEOUT_SECONDS` if needed
- Consider adding a `TWOCAPTCHA_SOFTWARE_KEY` for priority processing
- Check 2Captcha's current server load

**High error rate:**
- Some CAPTCHAs are more difficult than others
- 2Captcha typically has >95% success rate for reCAPTCHA v2
- Consider increasing `CAPTCHA_MAX_RETRIES` for difficult cases
